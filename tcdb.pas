...
function tcadbnew :pTCADB; cdecl;external;


(* Delete an abstract database object.
   `adb' specifies the abstract database object. *)
procedure tcadbdel(adb:pTCADB); cdecl;external;
...
function tcadbopen(adb: pTCADB; name: pchar): cbool; cdecl;external;
...
function tcadbclose(adb: pTCADB): cbool; cdecl;external;
...
function tcadbput(adb: pTCADB; kbuf: pointer; ksiz: cint; vbuf: pointer; vsiz: cint): cbool; cdecl;external;
...
(* Store a string record into an abstract object.
   ...
   If successful, the return value is true, else, it is false.
   If a record with the same key exists in the database, it is overwritten. *)
function tcadbput2(adb: pTCADB; kstr:pchar ; vstr: pchar): cbool; cdecl;external;
...
function tcadbputkeep(adb: pTCADB; kbuf: pointer; ksiz: cint; vbuf: pointer; vsiz: cint): cbool; cdecl;external;
...
(* Store a new string record into an abstract database object.
   ...
   If successful, the return value is true, else, it is false.
   If a record with the same key exists in the database, this function has no effect. *)
function tcadbputkeep2(adb: pTCADB; kstr:pchar ; vstr: pchar): cbool; cdecl;external;
...
function tcadbputcat(adb: pTCADB; kbuf: pointer; ksiz: cint; vbuf: pointer; vsiz: cint): cbool; cdecl;external;
...
(* Concatenate a string value at the end of the existing record in an abstract database object.
   ...
   If successful, the return value is true, else, it is false.
   If there is no corresponding record, a new record is created. *)
function tcadbputcat2(adb: pTCADB; kstr:pchar ; vstr: pchar): cbool; cdecl;external;
...
(* Remove a record of an abstract database object.
   ...
   `ksiz' specifies the size of the region of the key.
   If successful, the return value is true, else, it is false. *)
function tcadbout(adb: pTCADB; kbuf:pointer; ksiz:cint): cbool; cdecl;external;
...
(* Remove a string record of an abstract database object.
   ...
   `kstr' specifies the string of the key.
   If successful, the return value is true, else, it is false. *)
function tcadbout2(adb: pTCADB; kstr: pchar): cbool; cdecl;external;
...
function tcadbget(adb: pTCADB; kbuf:pointer; ksiz: cint; sp: pcint): pointer; cdecl;external;
...
function tcadbget2(adb: pTCADB; kstr: pchar): pchar; cdecl;external;
...
(* Get the size of the value of a record in an abstract database object.
   ...
   it is -1. *)
function tcadbvsiz(adb: pTCADB; kbuf:pointer; ksiz:cint): cint; cdecl;external;
...
(* Get the next key string of the iterator of an abstract database object.
   ...
   access. *)
char *tcadbiternext2(adb: pTCADB);
...
TCLIST *tcadbfwmkeys(adb: pTCADB, const void *pbuf, int psiz, int max);
...
(* Get forward matching string keys in an abstract database object.
   ...
   deleted with the function `tclistdel' when it is no longer in use.  Note that this function
   may be very slow because every key in the database is scanned. *)
TCLIST *tcadbfwmkeys2(adb: pTCADB, const char *pstr, int max);
...
int tcadbaddint(adb: pTCADB, const void *kbuf, int ksiz, int num);
...
double tcadbadddouble(adb: pTCADB, const void *kbuf, int ksiz, double num);
}

(* Synchronize updated contents of an abstract database object with the file and the device.
   `adb' specifies the abstract database object.
   If successful, the return value is true, else, it is false. *)
function tcadbsync(adb: pTCADB): cbool; cdecl;external;
...
(* Optimize the storage of an abstract database object.
   ...
   successive updating. *)
function tcadboptimize(adb: pTCADB; params: pchar): cbool; cdecl;external;


(* Remove all records of an abstract database object.
   `adb' specifies the abstract database object.
   If successful, the return value is true, else, it is false. *)
function tcadbvanish(adb: pTCADB): cbool; cdecl;external;
...
(* Copy the database file of an abstract database object.
   ...
   the database file. *)
function tcadbcopy(adb: pTCADB; path: pchar): cbool; cdecl;external;
...
function tcadbtranbegin(adb: pTCADB): cbool; cdecl;external;
...
function tcadbtrancommit(adb: pTCADB): cbool; cdecl;external;
...
function tcadbtranabort(adb: pTCADB): cbool; cdecl;external;
...
(* Get the file path of an abstract database object.
   ...
   database. *)
function tcadbpath(adb: pTCADB): pchar; cdecl;external;
...
(* Get the number of records of an abstract database object.
   ...
   instance. *)
function tcadbrnum(adb: pTCADB): culonglong; cdecl;external;
...
(* Get the size of the database of an abstract database object.
   ...
   database instance. *)
function tcadbsize(adb: pTCADB): culonglong; cdecl;external;
...
TCLIST *tcadbmisc(adb: pTCADB, const char *name, const TCLIST *args);
}
...
(* Set an extra database sleleton to an abstract database object.
   ...
   If successful, the return value is true, else, it is false. *)
bool tcadbsetskel(adb: pTCADB, ADBSKEL *skel);
...
(* Set the multiple database skeleton to an abstract database object.
   ...
   If successful, the return value is true, else, it is false. *)
bool tcadbsetskelmulti(adb: pTCADB, int num);
...
bool tcadbputproc(adb: pTCADB, const void *kbuf, int ksiz, const void *vbuf, int vsiz,
...
(* Process each record atomically of an abstract database object.
   ...
   it is not needed, `NULL' can be specified.
   If successful, the return value is true, else, it is false. *)
bool tcadbforeach(adb: pTCADB, TCITER iter, void *op);
...
(* Map records of an abstract database object into another B+ tree database.
   ...
   If successful, the return value is true, else, it is false. *)
bool tcadbmapbdb(adb: pTCADB, TCLIST *keys, TCBDB *bdb, ADBMAPPROC proc, void *op, int64_t csiz);
...
function tcerrmsg(ecode: cint): pchar; cdecl;external;
